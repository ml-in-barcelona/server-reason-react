 ...
 *
 * ## Where does `react-client.js` come from?
 *
 ...
 *
 * So the sibling `packages/react-client` package builds it from the vendored React
 * submodule into a single self-contained file, `react-client.js` (generated by
 * `npm run react-client-generate` and committed). The dune rule here copies that
 * committed file in as a melange runtime dep, shipped next to this one and imported
 * relatively below. No npm dependency, no external fork, and building server-reason-react
 * needs neither Node nor the submodule — only regenerating react-client.js does.
 */

import ReactClientFlight from "./react-client.js";
