...
(** renderToStream renders a React tree into a Lwt_stream.t. The returned function aborts the stream: pending Suspense
    ...
    @param env
      Controls how much error detail is sent to the client on abort. [`Dev] (the default) includes the abort message in
      the $RX call; [`Prod] sends only the digest.
    @param identifier_prefix
      A prefix for IDs generated by [useId]. Similar to
      {:https://react.dev/reference/react-dom/server/renderToPipeableStream} *)

(** Resource hints, following react-dom's flight-side API. Each call emits a resource hint ([H] row) into the RSC
    payload, deduplicated per request. They work during [ReactServerDOM.render_model] and [ReactServerDOM.render_html]
    renders; they are no-ops in [renderToString]/[renderToStream] (no hint channel exists on those paths). See
    react_flight_spec/protocol.md for the wire grammar and scope. *)

val preload : href:string -> as_:string -> unit
val preconnect : href:string -> unit
val prefetchDNS : href:string -> unit
val preinitScript : href:string -> unit
