...
let domProps
  ...
  |> add (Option.map (fun v -> (String ("aria-keyshortcuts", v))) ariaKeyshortcuts)
  |> add (Option.map (fun v -> (String ("aria-label", v))) ariaLabel)
  |> add (Option.map (fun v -> (String ("aria-roledescription", v))) ariaRoledescription)
  |> add (Option.map (fun v -> (Bool ("aria-expanded", v))) ariaExpanded)
  |> add (Option.map (fun v -> (String ("aria-level", string_of_int v))) ariaLevel)
  |> add (Option.map (fun v -> (Bool ("aria-modal", v))) ariaModal)
  |> add (Option.map (fun v -> (Bool ("aria-multiline", v))) ariaMultiline)
  |> add (Option.map (fun v -> (Bool ("aria-multiselectable", v))) ariaMultiselectable)
  ...
  |> add (Option.map (fun v -> (String ("aria-valuemax", string_of_float v))) ariaValuemax)
  |> add (Option.map (fun v -> (String ("aria-valuemin", string_of_float v))) ariaValuemin)
  |> add (Option.map (fun v -> (String ("aria-valuenow", string_of_float v))) ariaValuenow)
  ...
  |> add (Option.map (fun v -> (String ("aria-activedescendant", v))) ariaActivedescendant)
  |> add (Option.map (fun v -> (String ("aria-colcount", string_of_int v))) ariaColcount)
  |> add (Option.map (fun v -> (String ("aria-colindex", string_of_int v))) ariaColindex)
  |> add (Option.map (fun v -> (String ("aria-colspan", string_of_int v))) ariaColspan)
  |> add (Option.map (fun v -> (String ("aria-controls", v))) ariaControls)
  |> add (Option.map (fun v -> (String ("aria-describedby", v))) ariaDescribedby)
  |> add (Option.map (fun v -> (String ("aria-errormessage", v))) ariaErrormessage)
  ...
  |> add (Option.map (fun v -> (String ("aria-posinset", string_of_int v))) ariaPosinset)
  |> add (Option.map (fun v -> (String ("aria-rowcount", string_of_int v))) ariaRowcount)
  |> add (Option.map (fun v -> (String ("aria-rowindex", string_of_int v))) ariaRowindex)
  |> add (Option.map (fun v -> (String ("aria-rowspan", string_of_int v))) ariaRowspan)
  |> add (Option.map (fun v -> (String ("aria-setsize", string_of_int v))) ariaSetsize)
  ...
  |> add (Option.map (fun v -> (Event ("onCompositionEnd", Composition v))) onCompositionEnd)
  |> add (Option.map (fun v -> (Event ("onCompositionStart", Composition v))) onCompositionStart)
  |> add (Option.map (fun v -> (Event ("onCompositionUpdate", Composition v))) onCompositionUpdate)
  ...
  |> add (Option.map (fun v -> (Event ("onPointerOver", Pointer v))) onPointerOver)
  |> add (Option.map (fun v -> (Event ("onPointerEnter", Pointer v))) onPointerEnter)
  |> add (Option.map (fun v -> (Event ("onPointerDown", Pointer v))) onPointerDown)
  |> add (Option.map (fun v -> (Event ("onPointerMove", Pointer v))) onPointerMove)
  |> add (Option.map (fun v -> (Event ("onPointerUp", Pointer v))) onPointerUp)
  |> add (Option.map (fun v -> (Event ("onPointerCancel", Pointer v))) onPointerCancel)
  |> add (Option.map (fun v -> (Event ("onPointerOut", Pointer v))) onPointerOut)
  |> add (Option.map (fun v -> (Event ("onPointerLeave", Pointer v))) onPointerLeave)
  |> add (Option.map (fun v -> (Event ("onGotPointerCapture", Pointer v))) onGotPointerCapture)
  |> add (Option.map (fun v -> (Event ("onLostPointerCapture", Pointer v))) onLostPointerCapture)
  ...
  |> add (Option.map (fun v -> (Event ("onCanPlayThrough", Media v))) onCanPlayThrough)
  |> add (Option.map (fun v -> (Event ("onDurationChange", Media v))) onDurationChange)
  ...
  |> add (Option.map (fun v -> (Event ("onLoadedMetadata", Media v))) onLoadedMetadata)
  ...
  |> add (Option.map (fun v -> (Event ("onVolumeChange", Media v))) onVolumeChange)
  |> add (Option.map (fun v -> (Event ("onWaiting", Media v))) onWaiting)
  |> add (Option.map (fun v -> (Event ("onAnimationStart", Animation v))) onAnimationStart)
  |> add (Option.map (fun v -> (Event ("onAnimationEnd", Animation v))) onAnimationEnd)
  |> add (Option.map (fun v -> (Event ("onAnimationIteration", Animation v))) onAnimationIteration)
  |> add (Option.map (fun v -> (Event ("onTransitionEnd", Transition v))) onTransitionEnd)
