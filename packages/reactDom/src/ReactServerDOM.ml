...
module Fiber = struct
  ...
  let push_hoisted_head_children ~fiber resource =
    fiber.hoisted_head_childrens <- fiber.hoisted_head_childrens @ [ Node resource ]
...
and render_lower_case_element ~fiber ~key ~tag ~attributes ~children =
  ...
  (* Unlike react, we don't reorder the elements in the head, we keep the original order to avoid hydration errors *)
  (* TODO: Reorder the elements in the head to fit react's behavior? *)
  if fiber.inside_head && not fiber.inside_body then
    ...
    | tag when (tag = "script" && is_async attributes) || (tag = "link" && has_precedence_and_rel_stylesheet attributes)
      ->
        render_head_element ~fiber ~key ~tag ~attributes ~children ~inner_html ~precedence:true
    | tag when tag = "title" || tag = "meta" || tag = "link" ->
        render_head_element ~fiber ~key ~tag ~attributes ~children ~inner_html ~precedence:false
    | _ -> render_regular_element ~fiber ~key ~tag ~attributes ~children ~inner_html

and render_head_element ~fiber ~key ~tag ~attributes ~children ~inner_html ~precedence =
  let props = Model.props_to_json attributes in
  let create_model_node children =
  ...
  let html_node = create_html_node ~html_props ~children_html in
  if precedence then Fiber.push_resource ~fiber html_node else Fiber.push_hoisted_head_children ~fiber html_node;
  Lwt.return (Html.null, create_model_node children_model)
...
let render_html ?(skipRoot = false) ?(env = `Dev) ?debug:(_ = false) ?bootstrapScriptContent ?bootstrapScripts
  ...
  let fiber : Fiber.t =
    { context; resources = initial_resources; inside_head = false; inside_body = false; hoisted_head_childrens = [] }
  in
  ...
  in
  let html_node = Html.Node.find_by_tag ~level:0. ~tag:"html" root_html in
  let html =
    let head_childrens = fiber.resources @ fiber.hoisted_head_childrens in
    match html_node with
    | Some html_node ->
        let body = Html.Node.find_by_tag ~tag:"body" (Html.list html_node.children) in
        (match (body, skipRoot) with
        | Some body, false -> Html.Node.append body (Html.list user_scripts)
        | None, false | None, true -> Html.Node.append html_node (Html.list user_scripts)
        | Some body, true ->
            body.children <- [];
            Html.Node.append html_node (Html.list user_scripts));
        (* Look for <head> that is in the root of <html> *)
        let head = Html.Node.find_by_tag ~level:0. ~tag:"head" (Html.list html_node.children) in
        (match head with
        (* To avoid hydration errors, we append the hoisted elements to the <head> element keeping the original order of head *)
        | Some head -> Html.Node.append head (Html.list head_childrens)
        | None ->
            (* If no explicit <head> was found, create one with the hoisted elements *)
            Html.Node.prepend html_node (Html.node "head" [] head_childrens));
        root_html
    | None -> if skipRoot then Html.list user_scripts else Html.list (head_childrens @ (root_html :: user_scripts))
  in
  let subscribe fn =
