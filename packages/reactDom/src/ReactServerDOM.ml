...
module Stream = struct
  ...
  let make ~env ~debug ~initial_index =
    let stream, push, close = Push_stream.make () in
    (stream, { push; close; env; debug; pending = 0; index = initial_index })
end
...
module Model = struct
  ...
  let to_chunk chunk id =
    match chunk with
    | Value value -> value_to_chunk id value
    | Debug_info_map debug_info -> debug_info_to_chunk id debug_info
  ...
  let rec element_to_payload ~context ~on_push element =
    ...
    let rec turn_element_into_payload element =
      ...
      | Upper_case_component (name, component) -> (
          ...
          | element ->
              (if context.debug then
                 let index = Stream.push ~context (on_push (Value (make_debug_info name))) in
                 let debug_info_ref = Printf.sprintf "$%x" index in
                 let _ = Stream.push ~context (on_push (Debug_info_map (`String debug_info_ref))) in
                 ());
              ...
              else turn_element_into_payload element
          | exception exn ->
              let error = exn_to_error exn in
              let index = Stream.push ~context (on_push (Error (context.env, error))) in
          ...
          | Sleep ->
              let promise =
                try%lwt
                  ...
                  Lwt.return (on_push (Value (turn_element_into_payload element)))
                with exn ->
                  let message = Printexc.to_string exn in
                  let stack = create_stack_trace () in
                  let error = make_error ~message ~stack ~digest:"" in
                  Lwt.return (on_push (Error (context.env, error)))
              in
      ...
      | Client_component { import_module; import_name; props; client = _ } ->
          ...
          let index = Stream.push ~context (on_push (Component_ref ref)) in
          let client_props = client_values_to_json ~context ~on_push props in
  ...
  and client_value_to_json ~context ~on_push value =
    match (value : React.client_value) with
    | Json json -> json
    | Error error ->
        let index = Stream.push ~context (on_push (Error (context.Stream.env, error))) in
        `String (error_value index)
    | Element element -> element_to_payload ~context ~on_push element
    | Promise (promise, value_to_json) -> (
              ...
              Lwt.map on_push
                (try%lwt
                 ...
                 with exn ->
                   ...
                   let stack = create_stack_trace () in
                   let error = make_error ~message ~stack ~digest:"" in
                   Lwt.return (Error (context.env, error)))
            in
            let index = Stream.push_async promise ~context in
        ...
        | Fail exn ->
            ...
            raise exn)
    | Function action ->
        let index = Stream.push ~context (on_push (Value (`Assoc [ ("id", `String action.id); ("bound", `Null) ]))) in
        `String (action_value index)

  and client_values_to_json ~context ~on_push props =
    List.map (fun (name, value) -> (name, client_value_to_json ~context ~on_push value)) props
end
...
module Fiber = struct
  ...
  type t = {
    context : Html.element Stream.context;
  ...
  let boundary_to_chunk index html =
  ...
  let rec client_to_html ~fiber ~on_push (element : React.element) =
    match element with
    | Empty -> Lwt.return Html.null
    | DangerouslyInnerHtml html -> Lwt.return (Html.raw html)
    | Text text -> Lwt.return (Html.string text)
    | Fragment children -> client_to_html ~on_push ~fiber children
    | List childrens ->
        let%lwt html = Lwt_list.map_p (client_to_html ~on_push ~fiber) childrens in
        Lwt.return (Html.list html)
    | Array childrens ->
        let%lwt html = childrens |> Array.to_list |> Lwt_list.map_p (client_to_html ~on_push ~fiber) in
        Lwt.return (Html.list html)
    | Lower_case_element { key; tag; attributes; children } ->
          ...
          List.map
            (fun prop ->
              ...
              | React.JSX.Action (_, key, f) ->
                  ...
                  let index = Stream.push ~context (on_push (Model (Value json))) in
                  React.JSX.String (key, key, Model.action_value index)
              | _ -> prop)
            attributes
        in
        render_lower_case ~fiber ~key ~tag ~attributes ~on_push ~children
    | Upper_case_component (_name, component) ->
        let rec wait_for_suspense_to_resolve () =
          ...
          | output -> client_to_html ~fiber ~on_push output
        in
        wait_for_suspense_to_resolve ()
    ...
    | Suspense { key = _; children; fallback } ->
        ...
        let%lwt fallback = client_to_html ~fiber ~on_push fallback in
        let context = get_context fiber in
        let async =
          let%lwt html = children |> client_to_html ~fiber ~on_push in
          Lwt.return (on_push (Suspense_Boundary html))
        in
    ...
    | Client_component { import_module = _; import_name = _; props = _; client } ->
        client_to_html ~fiber ~on_push (client ())
    | Provider children -> client_to_html ~fiber ~on_push children
    | Consumer children -> client_to_html ~fiber ~on_push children

  and render_lower_case ~fiber ~key:_ ~tag ~attributes ~on_push ~children =
    ...
    | None ->
        let%lwt html = Lwt_list.map_p (client_to_html ~fiber ~on_push) children in
  ...
  let rec render_element_to_html ~fiber ~on_push (element : React.element) : (Html.element * json) Lwt.t =
    match element with
    ...
    | Fragment children -> render_element_to_html ~fiber ~on_push children
    | List list -> elements_to_html ~fiber ~on_push list
    | Array arr -> elements_to_html ~fiber ~on_push (Array.to_list arr)
    | Upper_case_component (_name, component) -> (
        (* if debug then (
          ...
          ()); *)
        match component () with
        | element -> render_element_to_html ~fiber ~on_push element)
    | Async_component (_, component) ->
        let%lwt element = component () in
        render_element_to_html ~fiber ~on_push element
    | Client_component { import_module; import_name; props; client } ->
        let context = get_context fiber in
        let props =
          let on_push value = on_push (Model value) in
          Model.client_values_to_json ~context ~on_push props
        in
    ...
    | Suspense { key; children; fallback } -> (
        let context = get_context fiber in
        let%lwt html_fallback, model_fallback = render_element_to_html ~fiber ~on_push fallback in
        try%lwt
          let promise = render_element_to_html ~fiber ~on_push children in
          match Lwt.state promise with
          | Sleep ->
              let promise =
                Lwt.map on_push
                  (try%lwt
                     ...
                     let chunks = [ Suspense_Boundary html; Model (Value model) ] in
                     Lwt.return (Batch chunks)
                   with exn ->
                     let message = Printexc.to_string exn in
                     let stack = create_stack_trace () in
                     let error = make_error ~message ~stack ~digest:"" in
                     Lwt.return (Model (Error (context.env, error))))
              in
        ...
        with exn ->
          ...
          let chunks = [ Model (Error (context.env, error)); Suspense_Boundary Html.null ] in
          let index = Stream.push ~context (on_push (Batch chunks)) in
    ...
    | Provider children -> render_element_to_html ~fiber ~on_push children
    | Consumer children -> render_element_to_html ~fiber ~on_push children
    | Lower_case_element { key; tag; attributes; children } ->
        render_lower_case_element ~fiber ~on_push ~key ~tag ~attributes ~children

  and render_lower_case_element ~fiber ~on_push ~key ~tag ~attributes ~children =
    ...
    if fiber.inside_head && not fiber.inside_body then
      render_regular_element ~fiber ~on_push ~key ~tag ~attributes ~children ~inner_html
    else
      match tag with
      | "html" -> (
          ...
          | _ -> render_regular_element ~fiber ~on_push ~key ~tag ~attributes ~children ~inner_html)
      | "body" ->
          fiber.inside_body <- true;
          let%lwt value = render_regular_element ~fiber ~on_push ~key ~tag ~attributes ~children ~inner_html in
      ...
      | "head" ->
          ...
          let%lwt value =
            handle_hoistable_element ~fiber ~on_push ~key ~tag ~attributes ~children ~inner_html
      ...
      | tag
        ...
        ->
          handle_hoistable_element ~fiber ~on_push ~key ~tag ~attributes ~children ~inner_html ~hoist_html:push_resource
      | tag when tag = "title" || tag = "meta" || tag = "link" ->
          handle_hoistable_element ~fiber ~on_push ~key ~tag ~attributes ~children ~inner_html
            ~hoist_html:push_hoisted_head_childrens
      | _ -> render_regular_element ~fiber ~on_push ~key ~tag ~attributes ~children ~inner_html

  and handle_hoistable_element ~fiber ~on_push ~key ~tag ~attributes ~children ~inner_html ~hoist_html =
    ...
    let html_props = ReactDOM.attributes_to_html attributes in
    let%lwt children_html, children_model = elements_to_html ~fiber ~on_push children in
  ...
  and render_regular_element ~fiber ~key ~tag ~attributes ~on_push ~children ~inner_html =
    let context = get_context fiber in
    let html_props = ReactDOM.attributes_to_html attributes in
    let json_attributes =
      List.map
        (fun prop ->
          match prop with
          | React.JSX.Action (_, key, f) ->
              let html = on_push (Model (Value (`Assoc [ ("id", `String f.id); ("bound", `Null) ]))) in
    ...
    | false, None ->
        let%lwt html, model = elements_to_html ~fiber ~on_push children in
        Lwt.return (Html.node tag html_props [ html ], Model.node ~tag ~key ~props:json_props [ model ])

  and elements_to_html ~fiber ~on_push elements =
    let%lwt html_and_models = elements |> Lwt_list.map_p (render_element_to_html ~fiber ~on_push) in
...
let render_html ?(skipRoot = false) ?(env = `Dev) ?debug:(_ = false) ?bootstrapScriptContent ?bootstrapScripts
  ...
  let root_chunk = Fiber.to_chunk (Fiber.Model (Value root_model)) initial_index in
...
let render_model ?(env = `Dev) ?(debug = false) ?subscribe element =
  let stream, context = Stream.make ~env ~debug ~initial_index:1 in
  let root_payload = Model.element_to_payload ~context ~on_push:Model.to_chunk element in
  context.push (Model.to_chunk (Value root_payload) 0);
  if context.pending = 0 then context.close ();
...
let create_action_response ?(env = `Dev) ?(debug = false) ?subscribe response =
  ...
  in
  let stream, context = Stream.make ~env ~debug ~initial_index:1 in
  let json = Model.client_value_to_json ~context ~on_push:Model.to_chunk response in
  context.push (Model.to_chunk (Value json) 0);
  if context.pending = 0 then context.close ();
  match subscribe with None -> Lwt.return () | Some subscribe -> Lwt_stream.iter_s subscribe stream
