  ...
  assert_html (app ())
    ~shell:
      "<!--$?--><template id=\"B:1\"></template>Loading...<!--/$--><script \
       ...
       '>window.srr_stream.push()</script>"
    [
      "<div hidden id=\"S:1\"><span>Sleep resolved</span></div>\n\
...
let suspenasync_and_client () =
  ...
  assert_html (app ())
    ...
    [
      ...
      "<div hidden id=\"S:1\"><span>Only the client<!-- -->Part of async component</span></div>\n\
  ...
  assert_html (app ())
    ~shell:
      "<!--$?--><template id=\"B:1\"></template>Loading...<!--/$--><script \
       ...
       '>window.srr_stream.push()</script>"
    [
      "<div hidden id=\"S:1\"><div><section><article>Deep Server Content</article></section></div></div>\n\
  ...
  assert_html main ~disable_backtrace:true
    ...
    [
      "<script data-payload='1:E{\"message\":\"Failure(\\\"lol\\\")\",\"stack\":[],\"env\":\"Server\",\"digest\":\"\"}\n\
       '>window.srr_stream.push()</script><div hidden id=\"S:1\"></div>\n\
       <script>$RC('B:1', 'S:1')</script>";
    ]
  ...
  assert_html (app ())
    ~shell:
      "<!--$?--><template id=\"B:1\"></template>loading<!--/$--><script \
       ...
       '>window.srr_stream.push()</script>"
    [
      "<div hidden id=\"S:1\">from-provider</div>\n\
  ...
  assert_html (app ())
    ~shell:
      "<!--$?--><template id=\"B:1\"></template>loading<!--/$--><script \
       ...
       '>window.srr_stream.push()</script>"
    [
      "<div hidden id=\"S:1\"><span>outer-val+inner-val</span></div>\n\
       <script>$RC('B:1', 'S:1')</script><script \
...
let context_client_component_reads_context_across_async_suspense () =
  ...
  assert_html (app ())
    ...
    [
      "<script data-payload='2:I[\"./consumer.js\",[],\"Consumer\"]\n'>window.srr_stream.push()</script>";
      "<div hidden id=\"S:1\"><div>ctx-value</div></div>\n\
...
let timeout_emits_client_render_instruction_per_pending_boundary () =
  ...
  Alcotest.(check bool)
    ...
    (contains_substring html {|<!--$?--><template id="B:1"></template>Loading A<!--/$-->|});
  Alcotest.(check bool)
...
let timeout_with_late_resolving_boundary_does_not_crash () =
  ...
  let contains_substring str sub =
    match Str.search_forward (Str.regexp_string sub) str 0 with exception Not_found -> false | _ -> true
  in
  Alcotest.(check bool) "the pending boundary got a $RX call" true (contains_substring all_content {|$RX("B:1"|});
  Alcotest.(check bool)
