...
module Functions (F : Cstubs.FOREIGN) = struct
  ...
  let xxh3_create =
    F.foreign "XXH3_createState" (void @-> returning (ptr xxh3_state_t))
  ...
  module XXH3_64 = struct
    ...
    let hash =
      F.foreign "XXH3_64bits_withSeed"
        (string @-> size_t @-> ullong @-> returning ullong)
    ...
    let reset =
      F.foreign "XXH3_64bits_reset_withSeed"
        (ptr state_t @-> ullong @-> returning int)

    let update =
      F.foreign "XXH3_64bits_update"
        (ptr state_t @-> string @-> size_t @-> returning int)

    let digest =
      F.foreign "XXH3_64bits_digest" (ptr state_t @-> returning ullong)
