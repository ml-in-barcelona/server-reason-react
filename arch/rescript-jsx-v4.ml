...
let getJsxConfigByKey ~key ~type_ recordFields =
  let values =
    List.filter_map
      ...
      recordFields
  in
  match values with [] -> None | [ v ] | v :: _ -> Some v
  ...
  let jsxMapper ~config =
              ...
              -> (
                ...
                | _ ->
                    ...
                    Location.prerr_warning pattern.ppat_loc
                      (Preprocessor
                         (Printf.sprintf
                            "React: optional argument annotations must have \
    ...
    let argToConcreteType types (name, loc, type_) =
      ...
      | name when isOptional name ->
          ( getLabel name
          , []
          , Typ.constr ~loc { loc; txt = optionIdent } [ type_ ] )
          :: types
      | _ -> types
    in
        ...
        ; pstr_desc =
          ...
          | [ _ ] ->
              let rec getPropTypes types ({ ptyp_loc; ptyp_desc } as fullType) =
                match ptyp_desc with
                | Ptyp_arrow
                    (name, type_, ({ ptyp_desc = Ptyp_arrow _ } as rest))
              ...
              let namedTypeList =
                List.fold_left argToConcreteType [] propTypes
              in
            ...
            if hasAttrOnBinding binding then
              ...
              let modifiedBindingOld binding =
                ...
                let rec spelunkForFunExpression expression =
                  ...
                  | { pexp_desc =
                        Pexp_sequence
                          (_wrapperExpression, innerFunctionExpression)
                    } ->
                      spelunkForFunExpression innerFunctionExpression
              ...
              let modifiedBinding binding =
                ...
                let rec spelunkForFunExpression expression =
                  ...
                  | { pexp_desc =
                        Pexp_fun
                          (_nolabel, _default, pattern, _internalExpression)
                    } ->
                  ...
                  | { pexp_desc =
                      ...
                      in
                      ( (fun exp ->
                          Exp.apply wrapperExpression [ (nolabel, exp) ])
                      , hasUnit
                      , exp )
                  | { pexp_desc =
              ...
              in
              let bindingWrapper, hasUnit, expression =
    ...
    let transformJsxCall mapper callExpression callArguments attrs =
      ...
      | Pexp_ident caller -> (
          ...
          | { loc; txt = Ldot (modulePath, ("createElement" | "make")) } -> (
              ...
              | 3 ->
                  transformUppercaseCall3 modulePath mapper loc attrs
...
module V4 = struct
  ...
  *)
    ...
    | "automatic" ->
        ...
        let jsxExpr, key =
          ...
          | None, (_, keyExpr) :: _ ->
              ( Exp.ident
                  { loc = Location.none
                  ; txt = Ldot (Lident "React", "jsxKeyed")
                  }
              , [ (nolabel, keyExpr) ] )
          | None, [] ->
              ( Exp.ident
                  { loc = Location.none; txt = Ldot (Lident "React", "jsx") }
              , [] )
          | Some _, (_, keyExpr) :: _ ->
              ( Exp.ident
                  { loc = Location.none
                  ; txt = Ldot (Lident "React", "jsxsKeyed")
                  }
              , [ (nolabel, keyExpr) ] )
          | Some _, [] ->
              ( Exp.ident
                  { loc = Location.none; txt = Ldot (Lident "React", "jsxs") }
              , [] )
        in
    ...
    | "automatic" ->
        ...
        let jsxExpr, key =
          ...
          | None, (_, keyExpr) :: _ ->
              ( Exp.ident
                  { loc = Location.none
                  ; txt = Ldot (Lident "ReactDOM", "jsxKeyed")
                  }
              , [ (nolabel, keyExpr) ] )
          | None, [] ->
              ( Exp.ident
          ...
          | Some _, (_, keyExpr) :: _ ->
              ( Exp.ident
                  { loc = Location.none
                  ; txt = Ldot (Lident "ReactDOM", "jsxsKeyed")
                  }
              , [ (nolabel, keyExpr) ] )
          | Some _, [] ->
              ( Exp.ident
                  { loc = Location.none
                  ; txt = Ldot (Lident "ReactDOM", "jsxs")
                  }
              , [] )
        in
        Exp.apply ~attrs jsxExpr
  ...
  let argToType
      ~newtypes
      ~(typeConstraints : core_type option)
      types
    ...
    | None, name, _ when isOptional name ->
        ...
        :: types
    | None, name, _ when isLabelled name ->
        (false, getLabel name, [], Typ.var ~loc (safeTypeFromValue name))
        :: types
    | _ -> types
    [@@raises Invalid_argument]
  ...
  let transformStructureItem ~config mapper item =
    ...
    | { pstr_loc; pstr_desc = Pstr_value (recFlag, valueBindings) } -> (
                    ...
                    if isLabelled arg_label || isOptional arg_label then
                      returnedExpression
                        (( { loc = ppat_loc; txt = Lident (getLabel arg_label) }
                         , { pattern with
                             ppat_attributes =
                               (if isOptional arg_label then optionalAttr
                               else [])
              ...
              let bindings, newBinding =
                ...
                | Nonrecursive ->
                    ( [ { binding with
                          pvb_expr = expression
                        ; pvb_pat =
                            Pat.var { txt = fnName; loc = Location.none }
                        }
                      ]
                    , Some (bindingWrapper fullExpression) )
              in
        ...
        let structuresAndBinding = List.map mapBinding valueBindings in
        let otherStructures
            (type_, binding, newBinding)
            (types, bindings, newBindings) =
          let types =
            match type_ with Some type_ -> type_ :: types | None -> types
          in
          let newBindings =
            match newBinding with
            | Some newBinding -> newBinding :: newBindings
  ...
  let transformJsxCall
    ...
    | Pexp_ident caller -> (
        ...
        (* Foo.createElement(~prop1=foo, ~prop2=bar, ~children=[], ()) *)
        | { loc; txt = Ldot (modulePath, ("createElement" | "make")) } ->
            transformUppercaseCall3 ~config modulePath mapper jsxExprLoc loc
              attrs callArguments
        (* div(~prop1=foo, ~prop2=bar, ~children=[bla], ()) *)
        (* turn that into
           ReactDOMRe.createElement(~props=ReactDOMRe.props(~props1=foo, ~props2=bar, ()), [|bla|]) *)
