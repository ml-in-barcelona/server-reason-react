...
* Add a type-safe generated router: a `Router.make` manifest (typed path parameters like `/notes/:id<NoteId.t>`, per-scope typed search parameters, module attachments for pages/layouts/loading/error/not-found, loaders, metadata and headers) is compiled by `server-reason-react.router-gen` into a universal `Router` module (typed destinations, `Link`s, `href`s, `useRoute`/`useNavigation`/`useSearch`/`useUpdateHash`) and a native registry for `server-reason-react.router-native`. Navigation is negotiated over headers (`SRR-Navigation-From`, `SRR-Registry`, `SRR-Base-Revision`): the server computes the reusable layout prefix itself and answers with a full model or a partial Flight patch (`SRR-Response: full | patch | redirect | reload-required`), degrades to a full response when navigation facts are missing, answers `reload-required` on registry fingerprint mismatches, and stamps `Cache-Control: private, no-store` on every RSC response. The route matcher decodes paths in a single pass and is covered by `make bench-router`. A Dream adapter lives in the demo (`DreamRouter.routes`) by @davesnx
