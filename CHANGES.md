...
* Align the Flight wire format with react-server-dom-webpack 19.1.0 (all verified byte-for-byte by the flight spec): escape user strings starting with `$` (`"$foo"` → `"$$foo"`), serialize numeric JSX props as JSON numbers instead of strings, reference client components lazily with `$L<id>`, outline the suspense symbol as a deduplicated row, and emit prod element rows as 4-tuples `["$",type,key,props]` (dev keeps the 7-tuple debug form) by @davesnx
