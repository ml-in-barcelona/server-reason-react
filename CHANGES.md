...
* Render the styled-ppx carrier's label as the element's `part` attribute when expanding `styles=` on lowercase elements, in native and Melange mode alike: `<div styles=x />` now also emits `part` from `CSS.label x` whenever the label is non-empty, and an explicit `part` prop merges with it the way `className` does. Needs a styled-ppx runtime that defines `CSS.label`; on the Melange side `ReactDOM.domProps` has to accept `~part` by @davesnx
