...
* Demo: add `/demo/fragmentRSC` (a fragment-root RSC page showing head hoisting without an `<html>` wrapper and JavaScript-identical `Js.String`/`Js.Date` output), a client component that errors during SSR and recovers in the browser (`ThrowingClient` on `/demo/singlePageRSC`), and wire `DEMO_ENV=development` into `render_html`'s `~debug` by @davesnx
