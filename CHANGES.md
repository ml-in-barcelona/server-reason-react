...
* Fix `React.Children.map` and `mapWithIndex` to key mapped children like React (`.0`, `.1`, `.$key`, with a `<mapped key>/` prefix when the callback changes the key), so a React client no longer reports them as missing keys by @davesnx
