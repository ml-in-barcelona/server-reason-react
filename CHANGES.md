...
* Preserve static child positions in Flight output so development clients report missing keys in runtime collections without warning about explicit siblings. Children of a host element and members of a literal JSX sibling group are static; members of `React.list` and `React.array` are runtime collection members. Both environments now send seven-field element tuples; `Prod` still redacts error details. This adds 12 uncompressed bytes per production element tuple.
* Add `React.Static_children` to the native element representation: the literal sibling group the PPX builds for `<Component> a b </Component>` and fragments, in place of `React.list`. Direct element matches must handle the constructor.
* `React.Children.map` and `mapWithIndex` key their results like React (`.0`, `.1`, `.$key`), so mapped children no longer trigger missing-key warnings on development clients.
* Add `?timeout` and `?abort` to `ReactServerDOM.render_model`, `render_model_value`, and `create_action_response`, and `?abort` to `render_html`. The first timeout or host abort signal settles pending Flight rows as errors, adds `$RX` instructions for pending HTML Suspense boundaries, closes the stream, and cancels pending Lwt work where possible; late output from non-cancelable work is dropped. Model deadlines include action execution, and the Dream demo starts its action deadline before dispatch. `create_action_response` without a subscriber still waits for action settlement, ordinary action failures keep their wire format, and failed subscribers or canceled render promises cancel pending work by @davesnx
* Add a type-safe generated router: a `Router.make` manifest compiles through `server-reason-react.router-gen` into a universal `Router` module and a native registry for `server-reason-react.router-native`. It provides typed destinations and route, navigation, and search hooks; per-scope layouts, boundaries, loaders, metadata, and headers; full or partial Flight navigation responses; registry mismatch reloads; and private, non-cacheable RSC responses. Registry generation rejects equal-specificity overlaps, including crossing catch-all routes. Scalar path serializers must produce one valid segment, and nested path or search decode failures render the nearest ancestor error boundary before loaders run. The repository includes a demo-only Dream adapter, which is not installed by a published package by @davesnx
* Add `?nonce` to `ReactServerDOM.render_html`: every streamed script (RSC payload chunks, Suspense boundary instructions, the stream bootstrap and close scripts, and the bootstrap scripts/modules/content) carries the CSP nonce by @davesnx
* Fix the RSC model renderer to retry components, including root model components, suspended by `React.Experimental.usePromise` when their promise resolves instead of serializing the suspension as an error row by @davesnx
