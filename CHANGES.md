...
* Preserve static child positions in Flight output so development clients report missing keys in runtime collections without warning about explicit siblings. Both environments now send seven-field element tuples; `Prod` still redacts error details. This adds 12 uncompressed bytes per production element tuple.
* Add `React.Static_child` to the native element representation. Direct element matches must handle the marker. The PPX preserves static JSX positions and explicitly typed element child props while retaining custom child-prop types.
