...
let refType loc = [%type: React.domRef]
  ...
  let transformComponentDefinition mapper structure returnStructures =
    ...
    | { pstr_loc
      ; pstr_desc =
        ...
        transformExternal ~loc:pstr_loc ~pval_attributes ~pval_type ~fnName
          ~value_description pstr structure returnStructures
    (* let component = ... *)
    | { pstr_loc; pstr_desc = Pstr_value (recFlag, valueBindings) } ->
        transformComponent ~loc:pstr_loc mapper recFlag valueBindings
          nestedModules returnStructures
