...
module RequestContext : sig
  (** {2 Reading request data}

      These functions are available in both server components (render phase) and server functions (action phase). They
      raise if called outside a request context. *)
  ...
  (** {2 Writing cookies}
      ...
      Only available during the action phase (server functions). Raises during render or outside a request context. *)
...
(** Render a React element as a full HTML page or RSC model stream, depending on the request's [Accept] header.
    ...
    Installs a render-phase request context: [RequestContext.get_*] is available, [RequestContext.set_cookie] raises.
...
(** Handle a server function POST request.
    ...
    Installs an action-phase request context: both [RequestContext.get_*] and [RequestContext.set_cookie] are available.
