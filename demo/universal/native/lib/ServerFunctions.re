// The user will not use this, it'll be present after generated by the ppx
[@platform native]
let createError = (exn: exn) => {
  let message = Printexc.to_string(exn);
  let env =
    Sys.getenv_opt("DEMO_ENV") |> Option.value(~default="development");
  let stack =
    Printexc.backtrace_slots(Printexc.get_callstack(9))
    |> Option.get
    |> Array.to_list
    // Remove the first element because it's the current function
    |> List.tl;
  ...
  let stack =
    `List(
      List.map(
        slot => {
          let location = Printexc.Slot.location(slot);
          let name = Printexc.Slot.name(slot);
          ...
          | (_, _) => `List([`List([`String("Unknown stack")])])
          };
        },
        stack,
      ),
...
module Samples = {
  ...
  */
  ...
  [@platform native]
  let error = () => {
    failwith(Printf.sprintf("Error from server"));
  };
  ...
  let errorRouteHandler = _args =>
    try(error() |> Lwt.map(response => response)) {
    | e => Lwt.return(createError(e))
    };
