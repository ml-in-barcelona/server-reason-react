/*
   Due to the issues that you can see at create (line 71), edit (line 136) and delete (line 192) functions we will have the args always as a list of a single list of args.
...
module Notes = {
  ...
  // This the router  handler that will handle parsing the args and calling the handler the user declared
  ...
  let create =
    switch%platform () {
    | Server => Router.demoActionCreateNote
    | Client => (
        (~title, ~content) => {
          ...
          /**
            Under the hoods if we call action(id, title, content), melange will do this:
                function create(title, content) {
                  var action = ReactServerDOMWebpack.createServerReference(Router.demoActionCreateNote, "create");
  ...
  let edit =
    switch%platform () {
    | Server => Router.demoActionEditNote
    | Client => (
        (~id, ~title, ~content) => {
          let action =
            ReactServerDOMWebpack.createServerReference(
          ...
          /**
            Under the hoods if we call action(id, title, content), melange will do this:
                function edit(id, title, content) {
                  var action = ReactServerDOMWebpack.createServerReference(Router.demoActionEditNote, "edit");
  ...
  let delete =
    switch%platform () {
    | Server => Router.demoActionDeleteNote
    | Client => (
        id => {
          let action =
            ReactServerDOMWebpack.createServerReference(
          ...
          /**
            Under the hoods if we call action(id, title, content), melange will do this:
                function edit(id) {
                  var action = ReactServerDOMWebpack.createServerReference(Router.demoActionDeleteNote, "delete");
...
module Samples = {
  ...
  let formData =
    switch%platform () {
    | Server => Router.demoActionFormDataSample
    | Client => (
        formData => {
          let action =
            ReactServerDOMWebpack.createServerReference(
  ...
  // QUESTION: Should we handle decoding the formData? How could we do it?
  // For now, I'm letting for the user handle it by calling the value from Hashtbl
  // We can think more about it in the future when creating the ppx
  [@platform native]
  ...
  // The purpose of this action is to show how to handle server actions behave by passing them as a prop on a server component
  let formDataServerOnly = Router.demoActionFormDataServerOnly;
  ...
  let simpleResponse =
    switch%platform () {
    | Server => Router.demoActionSimpleResponse
    | Client => (
        _ => {
          let action =
            ReactServerDOMWebpack.createServerReference(
