...
module Samples = {
  ...
  [@platform native]
  let formDataServerOnlyRouteHandler = (~request: _, formData) =>
  ...
  let formDataServerOnly = demoActionFormDataServerOnly;
  ...
  // As soon the user declare the action with a request, we pass it to the handle,
  // If not, the user can declare a normal action with the args.
  [@platform native]
  let simpleResponseWithRequestHandler = (~request, message) => {
    Dream.log("Logging: %s", message);
    Lwt.return_none;
  };

  [@platform native]
  let simpleResponseWithRequestRouteHandler = (~request, message) =>
    simpleResponseWithRequestHandler(~request, message);

  let simpleResponseWithRequest =
    switch%platform () {
    | Server => demoActionSimpleResponse
    | Client => (
        message => {
