          ...
          | _ =>
            failwith(
              Printf.sprintf(
                "Invalid arguments %s",
                args
                |> Array.map(Yojson.Basic.Util.to_string)
                |> Array.to_list
                |> String.concat(","),
              ),
            )
...
/**
  ...
  */
// It's going to be on top to this that we are going to generate the codes bellow
let simpleResponseId = "id/simpleResponse";

[@platform native]
let simpleResponseHandler = (~name, ~age) => {
...
| Client =>
  ...
  [%mel.raw
   {|
    // extract-server-function id/simpleResponse simpleResponse
    ''
    |}
  ]
};
...
FunctionReferences.register(
  formDataId,
  FormData(formData => formDataRouteHandler(formData)),
);
