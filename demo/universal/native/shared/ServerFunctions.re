...
let getSessionUser = (): Js.Promise.t(string) => {
  let name =
    DreamRouter.RequestContext.get_cookie("demo_user")
...
let getUserAgent = (): Js.Promise.t(string) => {
  let ua =
    DreamRouter.RequestContext.get_header("User-Agent")
...
let setSessionUser = (~name: string): Js.Promise.t(string) => {
  DreamRouter.RequestContext.set_cookie(~path="/", "demo_user", name);
