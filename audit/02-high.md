...
- **Follow-up divergence (2026-07-09, found while verifying in the demo):** `render_html` couples flight and fizz in one walk, so a client component whose SSR throws under a *server* Suspense rejects the model too (`E` row; the client reference is never emitted). React keeps them separate: flight serializes the reference regardless (client components aren't executed at the flight layer), and only the fizz boundary errors — the browser then retries with the real module and can succeed. With the current architecture the browser gets an error where React would recover. Workaround: put the Suspense boundary inside the client tree (that path is fully React-conformant, see the fixed behavior above and `demo/universal/native/shared/ThrowingClient.re`). Fixing the server-Suspense case requires decoupling the model from the HTML render of client subtrees (e.g. push the client ref before `client_to_html`, and on failure emit an intact model + errored HTML boundary) — related to design tension T4/T5 (2.25).
