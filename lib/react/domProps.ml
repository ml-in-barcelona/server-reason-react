...
(* type domRef type style

   type dangerouslySetInnerHTML = { __html: string };

   let domProps ?(key: string option) ?(ref: domRef option)

   ?(ariaDetails: string option) ?(ariaDisabled: bool option) ?(ariaHidden: bool
   option)

   ?(ariaKeyshortcuts: string option) ?(ariaLabel: string option)
   ?(ariaRoledescription: string option)

   ?(ariaExpanded: bool option)

   ?(ariaLevel: int option) ?(ariaModal: bool option) ?(ariaMultiline: bool
   option) ?(ariaMultiselectable: bool option)

   ?(ariaPlaceholder: string option)

   ?(ariaReadonly: bool option) ?(ariaRequired: bool option) ?(ariaSelected:
   bool option) ?(ariaSort: string option) ?(ariaValuemax: float option)
   ?(ariaValuemin: float option) ?(ariaValuenow: float option) ?(ariaValuetext:
   string option)
   ...
   ?(ariaGrabbed: bool option)

   ?(ariaActivedescendant: string option) ?(ariaColcount: int option)
   ?(ariaColindex: int option) ?(ariaColspan: int option) ?(ariaControls: string
   option) ?(ariaDescribedby: string option) ?(ariaErrormessage: string option)
   ?(ariaFlowto: string option) ?(ariaLabelledby: string option) ?(ariaOwns:
   string option) ?(ariaPosinset: int option) ?(ariaRowcount: int option)
   ?(ariaRowindex: int option) ?(ariaRowspan: int option) ?(ariaSetsize: int
   option)

   ?(defaultChecked: bool option) ?(defaultValue: string option)

   ?(accessKey: string option) ?(className: string option) ?(contentEditable:
   bool option) ?(contextMenu: string option) ?(dir: string option) ?(draggable:
   bool option) ?(hidden: bool option) ?(id: string option) ?(lang: string
   option) ?(role: string option) ?(style: style option) ?(spellCheck: bool
   option) ?(tabIndex: int option) ?(title: string option)

   ?(itemID: string option) ?(itemProp: string option) ?(itemRef: string option)
   ?(itemScope: bool option) ?(itemType: string option)

   ?(accept: string option) ?(acceptCharset: string option) ?(action: string
   option) ?(allowFullScreen: bool option) ?(alt: string option) ?(async: bool
   option) ?(autoComplete: string option) ?(autoCapitalize: string option)
   ?(autoFocus: bool option) ?(autoPlay: bool option) ?(challenge: string
   option) ?(charSet: string option) ?(checked: bool option) ?(cite: string
   option) ?(crossOrigin: string option) ?(cols: int option) ?(colSpan: int
   option) ?(content: string option) ?(controls: bool option) ?(coords: string
   option) ?(data: string option) ?(dateTime: string option) ?(default: bool
   option) ?(defer: bool option) ?(disabled: bool option) ?(download: string
   option) ?(encType: string option) ?(form: string option) ?(formAction: string
   option) ?(formTarget: string option) ?(formMethod: string option) ?(headers:
   string option) ?(height: string option) ?(high: int option) ?(href: string
   option) ?(hrefLang: string option) ?(htmlFor: string option) ?(httpEquiv:
   string option) ?(icon: string option) ?(inputMode: string option)
   ?(integrity: string option) ?(keyType: string option) ?(kind: string option)
   ?(label: string option) ?(list: string option) ?(loop: bool option) ?(low:
   int option) ?(manifest: string option) ?(max: string option) ?(maxLength: int
   option) ?(media: string option) ?(mediaGroup: string option) ?(method_:
   string option) (* as method *) ?(min: string option) ?(minLength: int option)
   ?(multiple: bool option) ?(muted: bool option) ?(name: string option)
   ?(nonce: string option) ?(noValidate: bool option) ?(open_: bool option) (*
   as open *) ?(optimum: int option) ?(pattern: string option) ?(placeholder:
   string option) ?(playsInline: bool option) ?(poster: string option)
   ?(preload: string option) ?(radioGroup: string option) ?(readOnly: bool
   option) ?(rel: string option) ?(required: bool option) ?(reversed: bool
   option) ?(rows: int option) ?(rowSpan: int option) ?(sandbox: string option)
   ?(scope: string option) ?(scoped: bool option) ?(scrolling: string option)

   ?(selected: bool option) ?(shape: string option) ?(size: int option) ?(sizes:
   string option) ?(span: int option) ?(src: string option) ?(srcDoc: string
   option) ?(srcLang: string option) ?(srcSet: string option) ?(start: int
   option) ?(step: float option) ?(summary: string option) ?(target: string
   option) ?(type_: string option) (* as type *) ?(useMap: string option)
   ?(value: string option) ?(width: string option) ?(wrap: string option)

   ?(onCopy: (ReactEvent.Clipboard.t -> unit) option) ?(onCut:
   (ReactEvent.Clipboard.t -> unit) option) ?(onPaste: (ReactEvent.Clipboard.t
   -> unit) option)
   ...
   ?(onKeyDown: (ReactEvent.Keyboard.t -> unit) option) ?(onKeyPress:
   (ReactEvent.Keyboard.t -> unit) option) ?(onKeyUp: (ReactEvent.Keyboard.t ->
   unit) option)

   ?(onFocus: (ReactEvent.Focus.t -> unit) option) ?(onBlur: (ReactEvent.Focus.t
   -> unit) option)

   ?(onChange: (ReactEvent.Form.t -> unit) option) ?(onInput: (ReactEvent.Form.t
   -> unit) option) ?(onSubmit: (ReactEvent.Form.t -> unit) option) ?(onInvalid:
   (ReactEvent.Form.t -> unit) option)

   ?(onClick: (ReactEvent.Mouse.t -> unit) option) ?(onContextMenu:
   (ReactEvent.Mouse.t -> unit) option) ?(onDoubleClick: (ReactEvent.Mouse.t ->
   unit) option) ?(onDrag: (ReactEvent.Mouse.t -> unit) option) ?(onDragEnd:
   (ReactEvent.Mouse.t -> unit) option) ?(onDragEnter: (ReactEvent.Mouse.t ->
   unit) option) ?(onDragExit: (ReactEvent.Mouse.t -> unit) option)
   ?(onDragLeave: (ReactEvent.Mouse.t -> unit) option) ?(onDragOver:
   (ReactEvent.Mouse.t -> unit) option) ?(onDragStart: (ReactEvent.Mouse.t ->
   unit) option) ?(onDrop: (ReactEvent.Mouse.t -> unit) option) ?(onMouseDown:
   (ReactEvent.Mouse.t -> unit) option) ?(onMouseEnter: (ReactEvent.Mouse.t ->
   unit) option) ?(onMouseLeave: (ReactEvent.Mouse.t -> unit) option)
   ?(onMouseMove: (ReactEvent.Mouse.t -> unit) option) ?(onMouseOut:
   (ReactEvent.Mouse.t -> unit) option) ?(onMouseOver: (ReactEvent.Mouse.t ->
   unit) option) ?(onMouseUp: (ReactEvent.Mouse.t -> unit) option)

   ?(onSelect: (ReactEvent.Selection.t -> unit) option)

   ?(onTouchCancel: (ReactEvent.Touch.t -> unit) option) ?(onTouchEnd:
   (ReactEvent.Touch.t -> unit) option) ?(onTouchMove: (ReactEvent.Touch.t ->
   unit) option) ?(onTouchStart: (ReactEvent.Touch.t -> unit) option)
   ?(onPointerOver: (ReactEvent.Pointer.t -> unit) option) ?(onPointerEnter:
   (ReactEvent.Pointer.t -> unit) option) ?(onPointerDown: (ReactEvent.Pointer.t
   -> unit) option) ?(onPointerMove: (ReactEvent.Pointer.t -> unit) option)
   ?(onPointerUp: (ReactEvent.Pointer.t -> unit) option) ?(onPointerCancel:
   (ReactEvent.Pointer.t -> unit) option) ?(onPointerOut: (ReactEvent.Pointer.t
   -> unit) option) ?(onPointerLeave: (ReactEvent.Pointer.t -> unit) option)
   ...
   ?(onAbort: (ReactEvent.Media.t -> unit) option) ?(onCanPlay:
   (ReactEvent.Media.t -> unit) option) ?(onCanPlayThrough: (ReactEvent.Media.t
   -> unit) option) ?(onDurationChange: (ReactEvent.Media.t -> unit) option)
   ?(onEmptied: (ReactEvent.Media.t -> unit) option) ?(onEncrypetd:
   (ReactEvent.Media.t -> unit) option) ?(onEnded: (ReactEvent.Media.t -> unit)
   option) ?(onError: (ReactEvent.Media.t -> unit) option) ?(onLoadedData:
   (ReactEvent.Media.t -> unit) option) ?(onLoadedMetadata: (ReactEvent.Media.t
   -> unit) option) ?(onLoadStart: (ReactEvent.Media.t -> unit) option)
   ?(onPause: (ReactEvent.Media.t -> unit) option) ?(onPlay: (ReactEvent.Media.t
   -> unit) option) ?(onPlaying: (ReactEvent.Media.t -> unit) option)
   ?(onProgress: (ReactEvent.Media.t -> unit) option) ?(onRateChange:
   (ReactEvent.Media.t -> unit) option) ?(onSeeked: (ReactEvent.Media.t -> unit)
   option) ?(onSeeking: (ReactEvent.Media.t -> unit) option) ?(onStalled:
   (ReactEvent.Media.t -> unit) option) ?(onSuspend: (ReactEvent.Media.t ->
   unit) option) ?(onTimeUpdate: (ReactEvent.Media.t -> unit) option)
   ?(onVolumeChange: (ReactEvent.Media.t -> unit) option) ?(onWaiting:
   ...
   ?(accentHeight: string option) ?(accumulate: string option) ?(additive:
   string option) ?(alignmentBaseline: string option) ?(allowReorder: string
   option) ?(alphabetic: string option) ?(amplitude: string option)
   ?(arabicForm: string option) ?(ascent: string option) ?(attributeName: string
   option) ?(attributeType: string option) ?(autoReverse: string option)
   ?(azimuth: string option) ?(baseFrequency: string option) ?(baseProfile:
   string option) ?(baselineShift: string option) ?(bbox: string option)
   ?(begin_: string option) (* as begin *) ?(bias: string option) ?(by: string
   option) ?(calcMode: string option) ?(capHeight: string option) ?(clip: string
   option) ?(clipPath: string option) ?(clipPathUnits: string option)
   ...
   ?(contentStyleType: string option) ?(cursor: string option) ?(cx: string
   option) ?(cy: string option) ?(d: string option) ?(decelerate: string option)
   ?(descent: string option) ?(diffuseConstant: string option) ?(direction:
   string option) ?(display: string option) ?(divisor: string option)
   ?(dominantBaseline: string option) ?(dur: string option) ?(dx: string option)
   ?(dy: string option) ?(edgeMode: string option) ?(elevation: string option)
   ?(enableBackground: string option) ?(end_: string option) (* as end *)
   ?(exponent: string option) ?(externalResourcesRequired: string option)
   ?(fill: string option) ?(fillOpacity: string option) ?(fillRule: string
   option) ?(filter: string option) ?(filterRes: string option) ?(filterUnits:
   string option) ?(floodColor: string option) ?(floodOpacity: string option)
   ?(focusable: string option) ?(fontFamily: string option) ?(fontSize: string
   option) ?(fontSizeAdjust: string option) ?(fontStretch: string option)
   ?(fontStyle: string option) ?(fontVariant: string option) ?(fontWeight:
   string option) ?(fomat: string option) ?(from: string option) ?(fx: string
   option) ?(fy: string option) ?(g1: string option) ?(g2: string option)
   ?(glyphName: string option) ?(glyphOrientationHorizontal: string option)
   ?(glyphOrientationVertical: string option) ?(glyphRef: string option)
   ?(gradientTransform: string option) ?(gradientUnits: string option)
   ?(hanging: string option) ?(horizAdvX: string option) ?(horizOriginX: string
   option) ?(ideographic: string option) ?(imageRendering: string option) ?(in_:
   string option) (* as in *) ?(in2: string option) ?(intercept: string option)
   ?(k: string option) ?(k1: string option) ?(k2: string option) ?(k3: string
   option) ?(k4: string option) ?(kernelMatrix: string option)
   ?(kernelUnitLength: string option) ?(kerning: string option) ?(keyPoints:
   string option) ?(keySplines: string option) ?(keyTimes: string option)
   ?(lengthAdjust: string option) ?(letterSpacing: string option)
   ?(lightingColor: string option) ?(limitingConeAngle: string option) ?(local:
   string option) ?(markerEnd: string option) ?(markerHeight: string option)
   ?(markerMid: string option) ?(markerStart: string option) ?(markerUnits:
   string option) ?(markerWidth: string option) ?(mask: string option)
   ?(maskContentUnits: string option) ?(maskUnits: string option)
   ?(mathematical: string option) ?(mode: string option) ?(numOctaves: string
   option) ?(offset: string option) ?(opacity: string option) ?(operator: string
   option) ?(order: string option) ?(orient: string option) ?(orientation:
   string option) ?(origin: string option) ?(overflow: string option)
   ?(overflowX: string option) ?(overflowY: string option) ?(overlinePosition:
   string option) ?(overlineThickness: string option) ?(paintOrder: string
   option) ?(panose1: string option) ?(pathLength: string option)
   ?(patternContentUnits: string option) ?(patternTransform: string option)
   ?(patternUnits: string option) ?(pointerEvents: string option) ?(points:
   string option) ?(pointsAtX: string option) ?(pointsAtY: string option)
   ?(pointsAtZ: string option) ?(preserveAlpha: string option)
   ?(preserveAspectRatio: string option) ?(primitiveUnits: string option) ?(r:
   string option) ?(radius: string option) ?(refX: string option) ?(refY: string
   option) ?(renderingIntent: string option) ?(repeatCount: string option)
   ?(repeatDur: string option) ?(requiredExtensions: string option)
   ?(requiredFeatures: string option) ?(restart: string option) ?(result: string
   option) ?(rotate: string option) ?(rx: string option) ?(ry: string option)
   ?(scale: string option) ?(seed: string option) ?(shapeRendering: string
   option) ?(slope: string option) ?(spacing: string option) ?(specularConstant:
   string option) ?(specularExponent: string option) ?(speed: string option)
   ?(spreadMethod: string option) ?(startOffset: string option) ?(stdDeviation:
   string option) ?(stemh: string option) ?(stemv: string option) ?(stitchTiles:
   string option) ?(stopColor: string option) ?(stopOpacity: string option)
   ?(strikethroughPosition: string option) ?(strikethroughThickness: string
   option) string option ?(stroke: string option) ?(strokeDasharray: string
   option) ?(strokeDashoffset: string option) ?(strokeLinecap: string option)
   ?(strokeLinejoin: string option) ?(strokeMiterlimit: string option)
   ?(strokeOpacity: string option) ?(strokeWidth: string option) ?(surfaceScale:
   string option) ?(systemLanguage: string option) ?(tableValues: string option)
   ?(targetX: string option) ?(targetY: string option) ?(textAnchor: string
   option) ?(textDecoration: string option) ?(textLength: string option)
   ?(textRendering: string option) ?(to_: string option) (* as to *)
   ?(transform: string option) ?(u1: string option) ?(u2: string option)
   ?(underlinePosition: string option) ?(underlineThickness: string option)
   ?(unicode: string option) ?(unicodeBidi: string option) ?(unicodeRange:
   string option) ?(unitsPerEm: string option) ?(vAlphabetic: string option)
   ?(vHanging: string option) ?(vIdeographic: string option) ?(vMathematical:
   string option) ?(values: string option) ?(vectorEffect: string option)
   ?(version: string option) ?(vertAdvX: string option) ?(vertAdvY: string
   option) ?(vertOriginX: string option) ?(vertOriginY: string option)
   ?(viewBox: string option) ?(viewTarget: string option) ?(visibility: string
   option) ?(widths: string option) ?(wordSpacing: string option) ?(writingMode:
   string option) ?(x: string option) ?(x1: string option) ?(x2: string option)
   ?(xChannelSelector: string option) ?(xHeight: string option) ?(xlinkActuate:
   string option) ?(xlinkArcrole: string option) ?(xlinkHref: string option)
   ?(xlinkRole: string option) ?(xlinkShow: string option) ?(xlinkTitle: string
   option) ?(xlinkType: string option) ?(xmlns: string option) ?(xmlnsXlink:
   string option) ?(xmlBase: string option) ?(xmlLang: string option)
   ?(xmlSpace: string option) ?(y: string option) ?(y1: string option) ?(y2:
   string option) ?(yChannelSelector: string option) ?(z: string option)
   ?(zoomAndPan: string option) ?(about: string option) ?(datatype: string
   option) ?(inlist: string option) ?(prefix: string option) ?(property: string
   option) ?(resource: string option) ?(typeof: string option) ?(vocab: string
   option) ?(dangerouslySetInnerHTML: dangerouslySetInnerHTML option)
