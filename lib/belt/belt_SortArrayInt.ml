...
let strictlySortedLength (xs : element array) =
  ...
  | _ ->
      ...
      if x0 < x1 then sortedLengthAuxLess xs x1 2 len
      else if x0 > x1 then -sortedLengthAuxMore xs x1 2 len
      else 1

let rec isSortedAux (a : element array) i last_bound =
  if i = last_bound then true
  else if A.getUnsafe a i <= A.getUnsafe a (i + 1) then
...
let rec binarySearchAux (arr : element array) lo hi key =
  ...
  if key = midVal then mid
    ...
    if hi = mid then if A.getUnsafe arr lo = key then lo else -(hi + 1)
