...
let rec copyBucket ~hash ~h_buckets ~ndata_tail old_bucket =
  ...
  | Some cell ->
      let nidx =
        (Belt_Id.getHashInternal hash) (N.key cell) land (A.length h_buckets - 1)
      in
...
let a = Belt_List.reverse [ 1; 2; 3 ]
let _ = Belt_List.map a (fun x -> print_int x)
...
let aa : int Js.undefined array = Belt_Array.makeUninitialized 1
let _ = print_endline @@ "size: " ^ string_of_int (Belt_Array.length aa)

let _ =
  Belt_Array.forEachU aa (fun x ->
      match Js.Undefined.toOption x with
      | None -> print_endline "YUP"
      | ((Some x) [@explicit_arity]) -> assert false)

let aa = Belt_Array.mapWithIndex aa (fun i _ -> i)
let aaa = Belt_List.concat [ 1.0; 2.0 ] [ 3.0; 4.0 ]

let _ =
  Belt_List.forEach aaa (fun x ->
